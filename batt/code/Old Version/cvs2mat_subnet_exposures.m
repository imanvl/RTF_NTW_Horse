...
function expos_mat = cvs2mat_subnet_exposures(filename, subnet_nodes)
...
% Col2 = lender Id, an integer from 1 to total number of nodes over dataset 
% Col3 = borrower Id, an integer from 1 to total number of nodes over dataset 
% Col4 = amount
...
%% exposure matrix
nr = length(subnet_nodes);
nc = length(subnet_nodes);
expos_mat = sparse(indI, indJ, w, nr, nc); % in case there are multiple links these are summed up
