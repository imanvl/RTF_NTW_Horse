...
%
%   annealingProfile:
%           Variable that describes how the annealing parameter evolves:
%           (1) increasing; (2) decreasing ; (3) inverted 'U' shaped;
%           (4) 'U' shaped
%
%   stopVal:
...
% Annealing parameter
annealingParameter = startingAnnealingLevel; 
% annealingProfile_fun(...
%     startingAnnealingLevel,...
%     deltaAnnealingLevel,...
%     0,...
%     annealingProfile)
...
% Links that are absent in the network
% absentLinksList_vec = sparse(...
%     reshape(~eye(N), 1, N^2) .* ...
%     (1:1:N^2) .* ...
%     reshape(...
%     repmat(((totalInterbankAssets_vec - ...
%     sum(estimatedInterbank_mat, 2)) > ...
%     prctile((totalInterbankAssets_vec - ...
%     sum(estimatedInterbank_mat, 2)), 0)),...
%     1, N),1, N^2)) .* ...
%     reshape(...
%     repmat(((totalInterbankLiabilities_vec - ...
%     sum(estimatedInterbank_mat)')' > ...
%     prctile((totalInterbankLiabilities_vec - ...
%     sum(estimatedInterbank_mat)')', 0))', ...
%     N, 1), 1, N^2);
...
%     fprintf('# links to pick = %i, %i\n',...
%         nnz(absentLinksList_vec), ...
%         nnz(priorQ_vec));
...
%              totalInterbankAssets_vec(linkRowPosition) ...
% 			 - sum(estimatedInterbank_mat(linkRowPosition,:)),...
%              totalInterbankLiabilities_vec(linkColumnPosition)...
% 			 - sum(estimatedInterbank_mat(:,linkColumnPosition)));
         
% 			 min(annealingParameter * ...
% 			 max(totalInterbankAssets_vec(linkRowPosition) ...
% 			 - sum(estimatedInterbank_mat(linkRowPosition,:)), ...
% 			 totalInterbankLiabilities_vec(linkColumnPosition)...
% 			 - sum(estimatedInterbank_mat(:,linkColumnPosition))), ...
% 			 min(totalInterbankAssets_vec(linkRowPosition) ...
...
%             fprintf('link (%i %i) added = %.2f\n', ...
%                 linkRowPosition, linkColumnPosition,...
%                 testInterbank_mat(linkRowPosition, ...
%                 linkColumnPosition));
            ...
            %             annealingParameter = annealingProfile_fun(...
            %                 startingAnnealingLevel,...
            %                 deltaAnnealingLevel,...
            %                 linksAddedCounter,...
            %                 annealingProfile);
...
%     fprintf('sum asset deficit = %.4f\niteration = %i\n\n', ...
%         100*sum(totalInterbankAssets_vec ...
%         - sum(estimatedInterbank_mat, 2))/sum(totalInterbankAssets_vec), iteration);
